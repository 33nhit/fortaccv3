...
import { CreditNoteLineItem, CreditNoteFormData, CustomerOption, VATOption } from '../../types/creditNote';

interface CreditNoteFormProps {
  onClose: () => void;
}

export const CreditNoteForm: React.FC<CreditNoteFormProps> = ({ onClose }) => {
  ...
  function createEmptyLineItem(): CreditNoteLineItem {
    return {
  ...
  function generateCustomerNumber(): string {
    ...
    return `CN${timestamp.slice(-6)}${random}`;
  }
  ...
  const handleLineItemChange = (lineIndex: number, field: keyof CreditNoteLineItem, value: any) => {
    const updatedLineItems = [...formData.lineItems];
    updatedLineItems[lineIndex] = {
      ...updatedLineItems[lineIndex],
      [field]: value,
  ...
  const generateCreditNote = (lineIndex: number) => {
    ...
    // Here you would implement the actual credit note generation logic
    alert(`Credit Note generated for ${lineItem.customerName} - ${lineItem.description}`);
  };
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-[95vw] max-h-[95vh] overflow-hidden m-4">
        <div className="p-6 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-gray-800">Credit Note Management</h2>
            <button
              onClick={onClose}
                  ...
                  <tr key={lineItem.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2 border-b">
                      <div className="flex space-x-1">
                        <button
                          onClick={() => generateCreditNote(index)}
          ...
          <div className="mt-6 bg-gray-50 p-4 rounded-lg">
            <div className="grid grid-cols-3 gap-4 text-right">
              ...
              <div>
                ...
                <span className="font-bold text-xl text-red-600">{getTotalAmount().toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
